...
var requestAPI = function(peerNames, channelName, chaincodeName, fcn, args, username, org) {
	...
	var scriptstring = util.format("{\"APIID\":%s,\"Certificate\":%s}",args[0],cer)
	sign.update(scriptstring);
	var sig = sign.sign(key, 'buffer');
	const rsSig = EcdsaDerSig.decode(sig, 'der');
    args[1] = rsSig.r.toString();
    args[2] = rsSig.s.toString();
    //开始异步请求
	return helper.getRegisteredUsers(username, org).then((user) => {
		...
		if (all_good) {
			logger.debug(util.format(
				...
				.signature));
			//加入chaincode的返回结果
			values[0] = proposalResponses[0].response.payload.toString();
			var request = {
		...
		if (response.status === 'SUCCESS') {
			...
			values[1] = tx_id.getTransactionID();
			return values;
		} else {
